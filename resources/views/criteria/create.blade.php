...
<div class="bg-white p-6 rounded-lg shadow-md max-w-lg mx-auto">
    ...
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Tambah Kriteria Baru untuk "{{ $decisionCase->name }}"</h1>

    <form action="{{ route('decision_cases.criteria.store', $decisionCase->id) }}" method="POST">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-6">
            ...
            <input type="number" name="weight" id="weight" step="0.01" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('weight') border-red-500 @enderror" value="{{ old('weight') }}" required min="0">
            @error('weight')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
